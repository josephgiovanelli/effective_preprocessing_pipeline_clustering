    ...
    with open(os.path.join(input_path, file_name)) as json_file:
        ...
        for elem in history:
            results = results.append(pd.DataFrame({
                ...
                'features__k': ['None' if (elem['pipeline']['features'][0] == 'features_NoneType' or elem['pipeline']['features'][0] == 'features_PearsonThreshold') else elem['pipeline']['features'][1]['features__k']], 
                'features__threshold': ['None' if elem['pipeline']['features'][0] != 'features_PearsonThreshold' else elem['pipeline']['features'][1]['features__threshold']], 
